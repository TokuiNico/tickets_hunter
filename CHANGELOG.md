...
- 改善 TixCraft 區域頁選區速度：掃描與點擊改為各一次瀏覽器往返（實測 183ms → 1.3ms），並移除點擊時注入頁面的紅點動畫
- 改善 TixCraft／KKTIX 每次輪詢的固定成本（cookie 橫幅、購票表單存在性探測），主迴圈實際週期由約 95ms 降至約 51ms
- 改善等待元素出現的反應時間，由 0.5 秒粒度改為 50ms
- Cloudflare 偵測不再把整頁 HTML 傳回程式端，改於頁面內判斷
- OCR 模型改為與瀏覽器啟動並行載入
- 新增 `benchmarks/` 與 `tests/`，可重現以上量測（詳見 `benchmarks/README.md`）
